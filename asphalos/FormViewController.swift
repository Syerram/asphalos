...
class FormViewController : UITableViewController {
    ...
    ///update row
    func updateRow(tag:String, callback:(row:FormRowDescriptor) -> ()) {
        var sectionIndex = 0
        var rowIndex = 0
    ...
    //hide row for the tag
    func hideRow(tag:String) {
        var sectionIndex = 0
        var rowIndex = 0
        ...
        for section in form.sections {
            for row in section.rows {
                if row.tag == tag {
                    var cell = self.tableView.cellForRowAtIndexPath(NSIndexPath(forRow: rowIndex, inSection: sectionIndex))
    ...
    private class func isSelectionableRowDescriptor(rowType: FormRowType) -> Bool {
        switch( rowType ) {
        case .Button, .BooleanCheck, .Picker, .Date, .Time, .DateAndTime, .ReadOnlyTextField:
            return true
        default:
            return false
        }
    ...
    private class func defaultCellClassForRowType(rowType: FormRowType) -> FormBaseCell.Type {
        dispatch_once(&Static.onceDefaultCellClass) {
            ...
            Static.defaultCellClasses[FormRowType.Textarea] = FormTextareaCell.self
            Static.defaultCellClasses[FormRowType.ReadOnlyTextField] = FormReadTextFieldCell.self
            Static.defaultCellClasses[FormRowType.MultiLineLabel] = FormMultiLineLabelCell.self
            Static.defaultCellClasses[FormRowType.PlainText] = FormPlainTextFieldCell.self
